<?php
/**
 * Embedr.eu Customizer functionality
 *
 * @package Embedr.eu
 ...
 */
function embedr_customize_register( $wp_customize ) {
	...
	$wp_customize->add_setting( 'color_scheme', array(
		'default'           => 'default',
		'sanitize_callback' => 'embedr_sanitize_color_scheme',
		'transport'         => 'postMessage',
	) );

	$wp_customize->add_control( 'color_scheme', array(
		'label'    => __( 'Base Color Scheme', 'embedr' ),
		'section'  => 'colors',
		'type'     => 'select',
		'choices'  => embedr_get_color_scheme_choices(),
		'priority' => 1,
	) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'sidebar_textcolor', array(
		'label'       => __( 'Header and Sidebar Text Color', 'embedr' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'embedr' ),
		'section'     => 'colors',
	) ) );
	...
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'header_background_color', array(
		'label'       => __( 'Header and Sidebar Background Color', 'embedr' ),
		'description' => __( 'Applied to the header on small screens and the sidebar on wide screens.', 'embedr' ),
		'section'     => 'colors',
	) ) );

	// Add an additional description to the header image section.
	$wp_customize->get_section( 'header_image' )->description = __( 'Applied to the header on small screens and the sidebar on wide screens.', 'embedr' );
}
add_action( 'customize_register', 'embedr_customize_register', 11 );

/**
 * Register color schemes for Embedr.eu.
 *
 * Can be filtered with {@see 'embedr_color_schemes'}.
 *
 ...
 */
function embedr_get_color_schemes() {
	return apply_filters( 'embedr_color_schemes', array(
		'default' => array(
			'label'  => __( 'Default', 'embedr' ),
			'colors' => array(
				'#f1f1f1',
				'#ffffff',
				'#ffffff',
...
if ( ! function_exists( 'embedr_get_color_scheme' ) ) :
/**
 * Get the current Embedr.eu color scheme.
 *
 * @since embedr.eu 1.0
 *
 * @return array An associative array of either the current or default color scheme hex values.
 */
function embedr_get_color_scheme() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	$color_schemes       = embedr_get_color_schemes();
...
endif; // embedr_get_color_scheme

if ( ! function_exists( 'embedr_get_color_scheme_choices' ) ) :
/**
 * Returns an array of color scheme choices registered for Embedr.eu.
 *
 * @since embedr.eu 1.0
 *
 * @return array Array of color schemes.
 */
function embedr_get_color_scheme_choices() {
	$color_schemes                = embedr_get_color_schemes();
...
endif; // embedr_get_color_scheme_choices

if ( ! function_exists( 'embedr_sanitize_color_scheme' ) ) :
/**
 ...
 */
function embedr_sanitize_color_scheme( $value ) {
	$color_schemes = embedr_get_color_scheme_choices();
...
endif; // embedr_sanitize_color_scheme
 ...
 */
function embedr_color_scheme_css() {
	$color_scheme_option = get_theme_mod( 'color_scheme', 'default' );
	...
	$color_textcolor_rgb         = embedr_hex2rgb( $color_scheme[3] );
	$color_sidebar_textcolor_rgb = embedr_hex2rgb( $color_scheme[4] );
	$colors = array(
	...
	$color_scheme_css = embedr_get_color_scheme_css( $colors );

	wp_add_inline_style( 'embedr-style', $color_scheme_css );
}
add_action( 'wp_enqueue_scripts', 'embedr_color_scheme_css' );
 ...
 * @since embedr.eu 1.0
 */
function embedr_customize_control_js() {
	...
	wp_localize_script( 'color-scheme-control', 'colorScheme', embedr_get_color_schemes() );
}
add_action( 'customize_controls_enqueue_scripts', 'embedr_customize_control_js' );
 ...
 * @since embedr.eu 1.0
 */
function embedr_customize_preview_js() {
	wp_enqueue_script( 'embedr-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20141216', true );
}
add_action( 'customize_preview_init', 'embedr_customize_preview_js' );
 ...
 */
function embedr_get_color_scheme_css( $colors ) {
 ...
 * @since embedr.eu 1.0
 */
function embedr_color_scheme_css_template() {
	$colors = array(
	...
	?>
	<script type="text/html" id="tmpl-embedr-color-scheme">
		<?php echo embedr_get_color_scheme_css( $colors ); ?>
	</script>
	<?php
}
add_action( 'customize_controls_print_footer_scripts', 'embedr_color_scheme_css_template' );
